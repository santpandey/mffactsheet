"""
Aditya Birla Sun Life Large & Mid Cap Fund - Downloader
Factsheet URL: https://mutualfund.adityabirlacapital.com/forms-and-downloads/factsheets
"""
...
EXCEL_DIR = ROOT_DIR / "excel-data" / "aditya-birla"


class AdityaBirlaDownloader(BaseFundDownloader):
    FUND_KEY = "aditya_birla"
    FUND_DISPLAY_NAME = "Aditya Birla Sun Life Large & Mid Cap Fund"
    BASE_DOMAIN = "https://mutualfund.adityabirlacapital.com"
    DOWNLOAD_DIR = EXCEL_DIR
    FUND_NAME_KEYWORDS = ["aditya birla", "birla", "large", "mid cap"]

    _PAGE_URL = "https://mutualfund.adityabirlacapital.com/forms-and-downloads/factsheets?page={page}"

    def get_output_filename(self, year: int, month: int) -> Path:
        month_name = MONTH_NAMES[month - 1]
        return EXCEL_DIR / f"absl-large-midcap-{month_name.lower()}-{year}.xlsx"
    ...
    def find_download_link(
        ...
        for link in all_links:
            ...
            is_fund = ("birla" in combined or "absl" in combined) and "large" in combined and "mid" in combined
        ...
        for link in excel_links:
            href = link.get("href", "")
            combined = (link.get_text(strip=True) + " " + href).lower()
            if ("birla" in combined or "absl" in combined) and "large" in combined and "mid" in combined:
                abs_url = make_absolute(href, self.BASE_DOMAIN)
