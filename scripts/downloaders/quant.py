"""
Quant Large and Mid Cap Fund - Downloader
Factsheet URL: https://quantmutual.com/statutory-disclosures
"""
...
EXCEL_DIR = ROOT_DIR / "excel-data" / "quant"


class QuantDownloader(BaseFundDownloader):
    FUND_KEY = "quant"
    FUND_DISPLAY_NAME = "Quant Large and Mid Cap Fund"
    BASE_DOMAIN = "https://www.quantmutual.com"
    DOWNLOAD_DIR = EXCEL_DIR
    FUND_NAME_KEYWORDS = ["quant", "large", "mid cap"]

    _PAGE_URL = "https://www.quantmutual.com/statutory-disclosures?page={page}"

    def get_output_filename(self, year: int, month: int) -> Path:
        month_name = MONTH_NAMES[month - 1]
        return EXCEL_DIR / f"quant-large-midcap-{month_name.lower()}-{year}.xlsx"
    ...
    def find_download_link(
        ...
        for link in all_links:
            ...
            is_fund = "quant" in combined and "large" in combined and "mid" in combined
        ...
        for link in excel_links:
            href = link.get("href", "")
            combined = (link.get_text(strip=True) + " " + href).lower()
            if "quant" in combined and "large" in combined and "mid" in combined:
                abs_url = make_absolute(href, self.BASE_DOMAIN)
