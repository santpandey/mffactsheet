"""
ICICI Prudential Large & Mid Cap Fund - Downloader
Factsheet URL: https://icicipruamc.com/downloads/factsheets
"""
...
EXCEL_DIR = ROOT_DIR / "excel-data" / "icici-prudential"


class ICICIPrudentialDownloader(BaseFundDownloader):
    FUND_KEY = "icici_prudential"
    FUND_DISPLAY_NAME = "ICICI Prudential Large & Mid Cap Fund"
    BASE_DOMAIN = "https://www.icicipruamc.com"
    DOWNLOAD_DIR = EXCEL_DIR
    FUND_NAME_KEYWORDS = ["icici", "large", "mid cap"]

    _PAGE_URL = "https://www.icicipruamc.com/downloads/factsheets?page={page}"

    def get_output_filename(self, year: int, month: int) -> Path:
        month_name = MONTH_NAMES[month - 1]
        return EXCEL_DIR / f"icici-large-midcap-{month_name.lower()}-{year}.xlsx"
    ...
    def find_download_link(
        ...
        for link in all_links:
            ...
            is_fund = "icici" in combined and "large" in combined and "mid" in combined
        ...
        for link in excel_links:
            href = link.get("href", "")
            combined = (link.get_text(strip=True) + " " + href).lower()
            if "icici" in combined and "large" in combined and "mid" in combined:
                abs_url = make_absolute(href, self.BASE_DOMAIN)
