"""
Bandhan Large & Mid Cap Fund - Downloader
Factsheet URL: https://bandhanmutual.com/downloads/factsheets
"""
...
EXCEL_DIR = ROOT_DIR / "excel-data" / "bandhan"


class BandhanDownloader(BaseFundDownloader):
    FUND_KEY = "bandhan"
    FUND_DISPLAY_NAME = "Bandhan Large & Mid Cap Fund"
    BASE_DOMAIN = "https://www.bandhanmutual.com"
    DOWNLOAD_DIR = EXCEL_DIR
    FUND_NAME_KEYWORDS = ["bandhan", "large", "mid cap"]

    _PAGE_URL = "https://www.bandhanmutual.com/downloads/factsheets?page={page}"

    def get_output_filename(self, year: int, month: int) -> Path:
        month_name = MONTH_NAMES[month - 1]
        return EXCEL_DIR / f"bandhan-large-midcap-{month_name.lower()}-{year}.xlsx"
    ...
    def find_download_link(
        ...
        for link in all_links:
            ...
            is_fund = "bandhan" in combined and "large" in combined and "mid" in combined
        ...
        for link in excel_links:
            href = link.get("href", "")
            combined = (link.get_text(strip=True) + " " + href).lower()
            if "bandhan" in combined and "large" in combined and "mid" in combined:
                abs_url = make_absolute(href, self.BASE_DOMAIN)
