"""
DSP Large & Mid Cap Fund - Downloader
Factsheet URL: https://dspim.com/downloads/factsheets
"""
...
EXCEL_DIR = ROOT_DIR / "excel-data" / "dsp"


class DSPDownloader(BaseFundDownloader):
    FUND_KEY = "dsp"
    FUND_DISPLAY_NAME = "DSP Large & Mid Cap Fund"
    BASE_DOMAIN = "https://www.dspim.com"
    DOWNLOAD_DIR = EXCEL_DIR
    FUND_NAME_KEYWORDS = ["dsp", "large", "mid cap"]

    _PAGE_URL = "https://www.dspim.com/downloads/factsheets?page={page}"

    def get_output_filename(self, year: int, month: int) -> Path:
        month_name = MONTH_NAMES[month - 1]
        return EXCEL_DIR / f"dsp-large-midcap-{month_name.lower()}-{year}.xlsx"
    ...
    def find_download_link(
        ...
        for link in all_links:
            ...
            is_fund = "dsp" in combined and "large" in combined and "mid" in combined
        ...
        for link in excel_links:
            href = link.get("href", "")
            combined = (link.get_text(strip=True) + " " + href).lower()
            if "dsp" in combined and "large" in combined and "mid" in combined:
                abs_url = make_absolute(href, self.BASE_DOMAIN)
