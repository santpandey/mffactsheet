"""
Kotak Large & Midcap Fund - Downloader
Factsheet URL: https://kotakmf.com/downloads/factsheet
"""
...
EXCEL_DIR = ROOT_DIR / "excel-data" / "kotak"


class KotakDownloader(BaseFundDownloader):
    FUND_KEY = "kotak"
    FUND_DISPLAY_NAME = "Kotak Large & Midcap Fund"
    BASE_DOMAIN = "https://www.kotakmf.com"
    DOWNLOAD_DIR = EXCEL_DIR
    FUND_NAME_KEYWORDS = ["kotak", "large", "midcap"]

    _PAGE_URL = "https://www.kotakmf.com/downloads/factsheet?page={page}"

    def get_output_filename(self, year: int, month: int) -> Path:
        month_name = MONTH_NAMES[month - 1]
        return EXCEL_DIR / f"kotak-large-midcap-{month_name.lower()}-{year}.xlsx"
    ...
    def find_download_link(
        ...
        for link in all_links:
            ...
            is_fund = "kotak" in combined and "large" in combined and "mid" in combined
        ...
        for link in excel_links:
            href = link.get("href", "")
            combined = (link.get_text(strip=True) + " " + href).lower()
            if "kotak" in combined and "large" in combined and "mid" in combined:
                abs_url = make_absolute(href, self.BASE_DOMAIN)
