"""
Axis Large & Mid Cap Fund - Downloader
Factsheet URL: https://axismf.com/download-center (Select Factsheet)
"""
...
EXCEL_DIR = ROOT_DIR / "excel-data" / "axis"


class AxisDownloader(BaseFundDownloader):
    FUND_KEY = "axis"
    FUND_DISPLAY_NAME = "Axis Large & Mid Cap Fund"
    BASE_DOMAIN = "https://www.axismf.com"
    DOWNLOAD_DIR = EXCEL_DIR
    FUND_NAME_KEYWORDS = ["axis", "large", "mid cap"]

    _PAGE_URL = "https://www.axismf.com/download-center?page={page}"

    def get_output_filename(self, year: int, month: int) -> Path:
        month_name = MONTH_NAMES[month - 1]
        return EXCEL_DIR / f"axis-large-midcap-{month_name.lower()}-{year}.xlsx"
    ...
    def find_download_link(
        ...
        for link in all_links:
            ...
            is_fund = "axis" in combined and "large" in combined and "mid" in combined
        ...
        for link in excel_links:
            href = link.get("href", "")
            combined = (link.get_text(strip=True) + " " + href).lower()
            if "axis" in combined and "large" in combined and "mid" in combined:
                abs_url = make_absolute(href, self.BASE_DOMAIN)
