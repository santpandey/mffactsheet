"""
Motilal Oswal Large and Midcap Fund - Downloader
Factsheet URL: https://motilaloswalmf.com/download-center/factsheets
"""
...
EXCEL_DIR = ROOT_DIR / "excel-data" / "motilal-oswal"


class MotilalOswalDownloader(BaseFundDownloader):
    FUND_KEY = "motilal_oswal"
    FUND_DISPLAY_NAME = "Motilal Oswal Large and Midcap Fund"
    BASE_DOMAIN = "https://www.motilaloswalmf.com"
    DOWNLOAD_DIR = EXCEL_DIR
    FUND_NAME_KEYWORDS = ["motilal", "oswal", "large", "midcap"]

    _PAGE_URL = "https://www.motilaloswalmf.com/download-center/factsheets?page={page}"

    def get_output_filename(self, year: int, month: int) -> Path:
        month_name = MONTH_NAMES[month - 1]
        return EXCEL_DIR / f"motilal-large-midcap-{month_name.lower()}-{year}.xlsx"
    ...
    def find_download_link(
        ...
        for link in all_links:
            ...
            is_fund = "motilal" in combined and "large" in combined and "mid" in combined
        ...
        for link in excel_links:
            href = link.get("href", "")
            combined = (link.get_text(strip=True) + " " + href).lower()
            if "motilal" in combined and "large" in combined and "mid" in combined:
                abs_url = make_absolute(href, self.BASE_DOMAIN)
