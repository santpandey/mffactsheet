"""
UTI Large & Mid Cap Fund - Downloader
Factsheet URL: https://utimf.com/forms-and-downloads (Select Factsheets)
"""
...
EXCEL_DIR = ROOT_DIR / "excel-data" / "uti"


class UTIDownloader(BaseFundDownloader):
    FUND_KEY = "uti"
    FUND_DISPLAY_NAME = "UTI Large & Mid Cap Fund"
    BASE_DOMAIN = "https://www.utimf.com"
    DOWNLOAD_DIR = EXCEL_DIR
    FUND_NAME_KEYWORDS = ["uti", "large", "mid cap"]

    _PAGE_URL = "https://www.utimf.com/forms-and-downloads?page={page}"

    def get_output_filename(self, year: int, month: int) -> Path:
        month_name = MONTH_NAMES[month - 1]
        return EXCEL_DIR / f"uti-large-midcap-{month_name.lower()}-{year}.xlsx"
    ...
    def find_download_link(
        ...
        for link in all_links:
            ...
            is_fund = "uti" in combined and "large" in combined and "mid" in combined
        ...
        for link in excel_links:
            href = link.get("href", "")
            combined = (link.get_text(strip=True) + " " + href).lower()
            if "uti" in combined and "large" in combined and "mid" in combined:
                abs_url = make_absolute(href, self.BASE_DOMAIN)
