"""
Invesco India Large & Mid Cap Fund - Downloader
Factsheet URL: https://invescomutualfund.com/literature-and-form
"""
...
EXCEL_DIR = ROOT_DIR / "excel-data" / "invesco"


class InvescoDownloader(BaseFundDownloader):
    FUND_KEY = "invesco"
    FUND_DISPLAY_NAME = "Invesco India Large & Mid Cap Fund"
    BASE_DOMAIN = "https://www.invescomutualfund.com"
    DOWNLOAD_DIR = EXCEL_DIR
    FUND_NAME_KEYWORDS = ["invesco", "large", "mid cap"]

    _PAGE_URL = "https://www.invescomutualfund.com/literature-and-form?page={page}"

    def get_output_filename(self, year: int, month: int) -> Path:
        month_name = MONTH_NAMES[month - 1]
        return EXCEL_DIR / f"invesco-large-midcap-{month_name.lower()}-{year}.xlsx"
    ...
    def find_download_link(
        ...
        for link in all_links:
            ...
            is_fund = "invesco" in combined and "large" in combined and "mid" in combined
        ...
        for link in excel_links:
            href = link.get("href", "")
            combined = (link.get_text(strip=True) + " " + href).lower()
            if "invesco" in combined and "large" in combined and "mid" in combined:
                abs_url = make_absolute(href, self.BASE_DOMAIN)
