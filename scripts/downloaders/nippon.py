"""
Nippon India Vision Fund (Large & Midcap) - Downloader
Factsheet URL: https://nipponindiamf.com/investor-services/downloads/factsheets
"""
...
EXCEL_DIR = ROOT_DIR / "excel-data" / "nippon"


class NipponDownloader(BaseFundDownloader):
    FUND_KEY = "nippon"
    FUND_DISPLAY_NAME = "Nippon India Vision Fund"
    BASE_DOMAIN = "https://www.nipponindiamf.com"
    DOWNLOAD_DIR = EXCEL_DIR
    FUND_NAME_KEYWORDS = ["nippon", "vision", "large", "mid"]

    _PAGE_URL = "https://www.nipponindiamf.com/investor-services/downloads/factsheets?page={page}"

    def get_output_filename(self, year: int, month: int) -> Path:
        month_name = MONTH_NAMES[month - 1]
        return EXCEL_DIR / f"nippon-vision-{month_name.lower()}-{year}.xlsx"
    ...
    def find_download_link(
        ...
        for link in all_links:
            ...
            is_fund = "nippon" in combined and ("vision" in combined or ("large" in combined and "mid" in combined))
        ...
        for link in excel_links:
            href = link.get("href", "")
            combined = (link.get_text(strip=True) + " " + href).lower()
            if "nippon" in combined and ("vision" in combined or ("large" in combined and "mid" in combined)):
                abs_url = make_absolute(href, self.BASE_DOMAIN)
