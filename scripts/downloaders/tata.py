"""
Tata Large & Mid Cap Fund - Downloader
Factsheet URL: https://tatamutualfund.com/downloads/factsheets
"""
...
EXCEL_DIR = ROOT_DIR / "excel-data" / "tata"


class TataDownloader(BaseFundDownloader):
    FUND_KEY = "tata"
    FUND_DISPLAY_NAME = "Tata Large & Mid Cap Fund"
    BASE_DOMAIN = "https://www.tatamutualfund.com"
    DOWNLOAD_DIR = EXCEL_DIR
    FUND_NAME_KEYWORDS = ["tata", "large", "mid cap"]

    _PAGE_URL = "https://www.tatamutualfund.com/downloads/factsheets?page={page}"

    def get_output_filename(self, year: int, month: int) -> Path:
        month_name = MONTH_NAMES[month - 1]
        return EXCEL_DIR / f"tata-large-midcap-{month_name.lower()}-{year}.xlsx"
    ...
    def find_download_link(
        ...
        for link in all_links:
            ...
            is_fund = "tata" in combined and "large" in combined and "mid" in combined
        ...
        for link in excel_links:
            href = link.get("href", "")
            combined = (link.get_text(strip=True) + " " + href).lower()
            if "tata" in combined and "large" in combined and "mid" in combined:
                abs_url = make_absolute(href, self.BASE_DOMAIN)
