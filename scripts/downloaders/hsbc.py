"""
HSBC Large and Mid Cap Fund - Downloader
Factsheet URL: https://assetmanagement.hsbc.co.in/en/mutual-funds/investor-resources/factsheet/
"""
...
EXCEL_DIR = ROOT_DIR / "excel-data" / "hsbc"


class HSBCDownloader(BaseFundDownloader):
    FUND_KEY = "hsbc"
    FUND_DISPLAY_NAME = "HSBC Large and Mid Cap Fund"
    BASE_DOMAIN = "https://assetmanagement.hsbc.co.in"
    DOWNLOAD_DIR = EXCEL_DIR
    FUND_NAME_KEYWORDS = ["hsbc", "large", "mid cap"]

    _PAGE_URL = (
        "https://assetmanagement.hsbc.co.in/en/mutual-funds/investor-resources/factsheet/?page={page}"
    )

    def get_output_filename(self, year: int, month: int) -> Path:
        month_name = MONTH_NAMES[month - 1]
        return EXCEL_DIR / f"hsbc-large-midcap-{month_name.lower()}-{year}.xlsx"
    ...
    def find_download_link(
        ...
        for link in all_links:
            ...
            is_fund = "hsbc" in combined and "large" in combined and "mid" in combined
        ...
        for link in excel_links:
            href = link.get("href", "")
            combined = (link.get_text(strip=True) + " " + href).lower()
            if "hsbc" in combined and "large" in combined and "mid" in combined:
                abs_url = make_absolute(href, self.BASE_DOMAIN)
