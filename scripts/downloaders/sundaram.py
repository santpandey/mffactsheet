"""
Sundaram Large and Mid Cap Fund - Downloader
Factsheet URL: https://sundarammutual.com/factsheets
"""
...
EXCEL_DIR = ROOT_DIR / "excel-data" / "sundaram"


class SundaramDownloader(BaseFundDownloader):
    FUND_KEY = "sundaram"
    FUND_DISPLAY_NAME = "Sundaram Large and Mid Cap Fund"
    BASE_DOMAIN = "https://www.sundarammutual.com"
    DOWNLOAD_DIR = EXCEL_DIR
    FUND_NAME_KEYWORDS = ["sundaram", "large", "mid cap"]

    _PAGE_URL = "https://www.sundarammutual.com/factsheets?page={page}"

    def get_output_filename(self, year: int, month: int) -> Path:
        month_name = MONTH_NAMES[month - 1]
        return EXCEL_DIR / f"sundaram-large-midcap-{month_name.lower()}-{year}.xlsx"
    ...
    def find_download_link(
        ...
        for link in all_links:
            ...
            is_fund = "sundaram" in combined and "large" in combined and "mid" in combined
        ...
        for link in excel_links:
            href = link.get("href", "")
            combined = (link.get_text(strip=True) + " " + href).lower()
            if "sundaram" in combined and "large" in combined and "mid" in combined:
                abs_url = make_absolute(href, self.BASE_DOMAIN)
