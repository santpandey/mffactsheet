"""
Franklin India Large & Mid Cap Fund - Downloader
Factsheet URL: https://franklintempletonindia.com/literature/factsheets
"""
...
EXCEL_DIR = ROOT_DIR / "excel-data" / "franklin"


class FranklinDownloader(BaseFundDownloader):
    FUND_KEY = "franklin"
    FUND_DISPLAY_NAME = "Franklin India Large & Mid Cap Fund"
    BASE_DOMAIN = "https://www.franklintempletonindia.com"
    DOWNLOAD_DIR = EXCEL_DIR
    FUND_NAME_KEYWORDS = ["franklin", "large", "mid cap"]

    _PAGE_URL = "https://www.franklintempletonindia.com/literature/factsheets?page={page}"

    def get_output_filename(self, year: int, month: int) -> Path:
        month_name = MONTH_NAMES[month - 1]
        return EXCEL_DIR / f"franklin-large-midcap-{month_name.lower()}-{year}.xlsx"
    ...
    def find_download_link(
        ...
        for link in all_links:
            ...
            is_fund = "franklin" in combined and "large" in combined and "mid" in combined
        ...
        for link in excel_links:
            href = link.get("href", "")
            combined = (link.get_text(strip=True) + " " + href).lower()
            if "franklin" in combined and "large" in combined and "mid" in combined:
                abs_url = make_absolute(href, self.BASE_DOMAIN)
